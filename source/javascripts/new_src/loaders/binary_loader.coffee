...
THREE.BinaryLoader = (showStatus) ->
  THREE.Loader.call this, showStatus

THREE.BinaryLoader:: = new THREE.Loader()
THREE.BinaryLoader::constructor = THREE.BinaryLoader
...
THREE.BinaryLoader::load = (url, callback, texturePath, binaryPath) ->
...
THREE.BinaryLoader::loadAjaxJSON = (context, url, callback, texturePath, binaryPath, callbackProgress) ->
...
THREE.BinaryLoader::loadAjaxBuffers = (json, callback, binaryPath, texturePath, callbackProgress) ->
  xhr = new XMLHttpRequest()
...
# Binary AJAX parser
THREE.BinaryLoader::createBinModel = (data, callback, texturePath, materials) ->
  vertex = (scope, x, y, z) ->
  ...
  f3n = (scope, normals, a, b, c, mi, na, nb, nc) ->
    nax = normals[na * 3]
    nay = normals[na * 3 + 1]
    naz = normals[na * 3 + 2]
    nbx = normals[nb * 3]
  ...
  f4n = (scope, normals, a, b, c, d, mi, na, nb, nc, nd) ->
    nax = normals[na * 3]
    nay = normals[na * 3 + 1]
    naz = normals[na * 3 + 2]
    nbx = normals[nb * 3]
  ...
  Model = (texturePath) ->
    ...
    normals = []
    uvs = []
    start_tri_flat = undefined
    start_tri_smooth = undefined
    start_tri_flat_uv = undefined
    start_tri_smooth_uv = undefined
    start_quad_flat = undefined
    start_quad_smooth = undefined
    start_quad_flat_uv = undefined
    start_quad_smooth_uv = undefined
    tri_size = undefined
    quad_size = undefined
    len_tri_flat = undefined
    len_tri_smooth = undefined
    len_tri_flat_uv = undefined
    len_tri_smooth_uv = undefined
    len_quad_flat = undefined
    len_quad_smooth = undefined
    len_quad_flat_uv = undefined
    len_quad_smooth_uv = undefined
    THREE.Geometry.call this
    THREE.Loader::initMaterials scope, materials, texturePath
    ...
    @computeCentroids()
    @computeFaceNormals()
    @computeTangents()  if THREE.Loader::hasNormals(this)
